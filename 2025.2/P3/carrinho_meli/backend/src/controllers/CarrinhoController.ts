...
export class CarrinhoController {
    ...
    inserir = async (req: Request, res: Response) => {
        ...
        try {
            ...
            res.status(201).json({message: "Produto inserido no carrinho", item: produto});
        } catch (error:any) {
    ...
    deletar = async (req: Request, res: Response) => {
        ...
        try {
            ...
            res.status(200).json({message: `Produto de id ${id} removido do carrinho`, item: produto});
        } catch (error:any) {
            if (error && error.message === 'Produto não encontrado') {
                return res.status(404).json({message: "Produto não encontrado no carrinho"});
            }
            return res.status(500).json({message: "Erro ao remover o produto do carrinho"});
        }
    }
    ...
    atualizar = async (req: Request, res: Response) => {
        ...
        try {
            ...
            res.status(200).json({message: `Produto de id ${id} atualizado no carrinho`, item: itemAtualizado});
        } catch (error:any) {
            if (error && error.message === 'Produto não encontrado') {
                return res.status(404).json({message: "Produto não encontrado no carrinho"});
            }
